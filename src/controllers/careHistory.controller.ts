...
export const addCareHistory = async (req: Request, res: Response) => {
  try {
    ...
    const { taskId, activity, description, performed_at, notes, images } =
      req.body;
    ...
    if (!activity || !description || !performed_at) {
      return res.status(400).json({
        success: false,
        message:
          "Vui lòng cung cấp loại hoạt động, mô tả và thời gian thực hiện",
      });
    }
...
export const getPlantCareHistory = async (req: Request, res: Response) => {
  try {
    const { plantId } = req.params;
    const userId = req.user?.id;
    const { page, limit, sortBy, sortOrder, startDate, endDate, activity } =
      req.query;
    ...
    return res.status(500).json({
      success: false,
      message:
        error instanceof Error ? error.message : "Lỗi khi lấy lịch sử chăm sóc",
    });
  }
};
  ...
  try {
    ...
    return res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Lỗi khi lấy thống kê lịch sử chăm sóc",
    });
  }
};
...
export const getCareHistoryDetail = async (req: Request, res: Response) => {
  try {
    ...
    const history = await careHistoryService.getCareHistoryById(
    ...
    return res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Lỗi khi lấy chi tiết lịch sử chăm sóc",
    });
  }
};
  ...
  try {
    ...
    return res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Lỗi khi cập nhật lịch sử chăm sóc",
    });
  }
};
...
export const deleteCareHistory = async (req: Request, res: Response) => {
  try {
    ...
    const result = await careHistoryService.deleteCareHistory(
      new mongoose.Types.ObjectId(historyId)
    );
    ...
    return res.status(500).json({
      success: false,
      message:
        error instanceof Error ? error.message : "Lỗi khi xóa lịch sử chăm sóc",
    });
  }
};
  ...
  try {
    ...
    return res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Lỗi khi đánh dấu công việc hoàn thành",
    });
  }
};
