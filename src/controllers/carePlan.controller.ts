  ...
  try {
    ...
    if (plant.carePlanId) {
      return res.status(400).json({
        success: false,
        message:
          "Cây trồng đã có kế hoạch chăm sóc, vui lòng cập nhật kế hoạch hiện tại",
      });
    }
...
export const getCarePlan = async (req: Request, res: Response) => {
  try {
    ...
    const carePlan = await carePlanService.getCarePlanByPlantId(
      new mongoose.Types.ObjectId(plantId)
    );
  ...
  try {
    ...
    return res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Lỗi khi cập nhật kế hoạch chăm sóc",
    });
  }
};
